...
import sidnet.stack.users.csgp.app.AgApp;
import sidnet.stack.users.csgp.routing.AggregateFixedRouting;
...
public static TopologyGUI topologyGUI = new TopologyGUI();
  ...
  public static Field createSim(int nodes, int length)
  {
    ...
    AgPlacement placement = new AgPlacement(bounds, nodes);
    ...
    for(int i=0; i<nodes; i++)
       myNode[i] = createNode(i, field, placement, protMap, radioInfoShared, pl, pl, simGUI.getSensorsPanelContext(), fieldContext, simManager, statistics, topologyGUI, sequenceNumberGlobal);
                                  ...
                                  SequenceGenerator sequenceNumber)
  {
    ...
    node.setGPS(gps);
     ...
     /* *** Configuring the ISO layers - more or less self-explanatory *** */
                ...
                /* APP <- Routing hookup */
                aggregateFixedRouting.setAppInterface(app.getAppProxy());
                ...
                aggregateFixedRouting.setNetEntity(net.getProxy());
                net.setProtocolHandler(Constants.NET_PROTOCOL_INDEX_1, aggregateFixedRouting.getProxy());

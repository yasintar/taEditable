...
public class AgApp implements AppInterface, CallbackInterface {
     ...
     * an infinite starvation loop */
      public void sensing(List params)
      {
           ...
           CSGPWrapperMessage msgSGP
           	= new CSGPWrapperMessage(msgDataValue, sinkLocation,
           							0, JistAPI.getTime());
    ...
    public void signalUserRequest()
    {
        ...
        {     
            ...
            myNode.getNodeGUI().colorCode.mark(colorProfileGeneric, CSGPColorProfile.SINK, ColorProfile.FOREVER); // to make easier to you to see the node you've posted the query through (the sink node)
          
            if (!query.isDispatched()) {        
                myNode.getNodeGUI().colorCode.mark(colorProfileGeneric, CSGPColorProfile.SINK, CSGPColorProfile.FOREVER);
        ...
        if (msg instanceof MessageQuery) { /* This is a source node. It receives the query request, and not it prepares to do the periodic sensing/sampling */
             ...
             if (msgQuery.getQuery() != null) { /* a query init message */
                if (!startedSensing) { /* To avoid creating duplicated sensing tasks due to duplicated requests, which may happen */
                    myNode.getNodeGUI().colorCode.mark(colorProfileGeneric, CSGPColorProfile.SOURCE, CSGPColorProfile.FOREVER);
